...
@php
    $detallesData = old('detalles');
    $detalles = is_array($detallesData) && count($detallesData) > 0 ? $detallesData : [['producto_id' => null, 'cantidad' => null, 'precio_unitario' => null]];
@endphp
...
<div class="container py-4">
    ...
    <div class="form-container">
        <div class="card-form">
            ...
            <form action="{{ route('facturas.store') }}" method="POST" onsubmit="return validarFormulario()">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <small class="text-muted">Este campo se llena automáticamente con el usuario actual.</small>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input id="total" type="number" step="0.01" name="total" value="{{ old('total') }}" class="form-control" required min="0" readonly style="background-color: #f8f9fa; font-weight: bold;">
                ...
                <div class="form-group">
                    ...
                    <select id="enviada_por_correo" name="enviada_por_correo" class="form-select" required>
                        <option value="0" {{ old('enviada_por_correo', '0') == '0' ? 'selected' : '' }}>No</option>
                    ...
                    <div id="detalles-container">
                        ...
                        <div class="detalle-item" data-index="{{ $index }}">
                            ...
                            <div class="form-group mb-0">
                                ...
                                <input id="detalle-precio-{{ $index }}" type="number" step="0.01" name="detalles[{{ $index }}][precio_unitario]" min="0" class="form-control" required placeholder="Precio unitario" value="{{ old("detalles.$index.precio_unitario") ?? $detalle['precio_unitario'] }}" readonly style="background-color: #f8f9fa; font-weight: bold;">
                                <small class="text-muted">El precio se obtiene automáticamente del producto seleccionado.</small>
                            </div>
                            <div class="form-group mb-0">
                                <label class="form-label">
                                    <i class="bi bi-calculator"></i> Subtotal
                                </label>
                                <div class="form-control subtotal-value" style="background-color: #f8f9fa; font-weight: bold;">$0.00</div>
                            </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        detalleItems.forEach(item => {
            ...
            if (productoSelect.value) {
                ...
                // Si no hay producto seleccionado, asegurarse de que el subtotal esté en cero
                const subtotalElement = item.querySelector('.subtotal-value');
                if (subtotalElement) {
        ...
        // Añadir event listeners a todos los selects de productos
        document.querySelectorAll('.producto-select').forEach(select => {
            select.addEventListener('change', function() {
                const index = this.closest('.detalle-item').dataset.index;
                actualizarPrecio(index);
            });
        });
        
        // Añadir event listeners a todos los inputs de cantidad
